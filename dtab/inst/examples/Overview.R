# =============================================================================
# Overview.R — simplified reference for the dtab package's functions
#
# Originally written against the pre-package Scripts_new/ folder (this is
# that same file, relocated into inst/examples/ once Scripts_new/ was
# packaged up as dtab - see calc_stats.R's own header note in the R/ folder
# for background on the Scripts/ vs Scripts_new/ split this package grew out
# of). Content is otherwise unchanged; only the Setup section below differs,
# now that loading the package replaces sourcing individual files.
#
...
#
# Scope: this covers the functions you actually call directly (dtab's
# exported API). It does NOT re-document every internal helper
# (standardise_names(), create_bases(), common_prefix_stem(), etc.) - see
# calc_stats.R's own comments in R/ for those; they're implementation
# details, not part of the day-to-day interface.
# =============================================================================


# ---- Setup ------------------------------------------------------------------
# library(dtab) is all you need for every function used below (calc_stats(),
# the pivot_*()/format_*() pairs, and the make_*() wrappers are all exported -
# see each function's own @export tag in R/). dplyr and tibble are loaded
# separately here purely so this script's own examples can use %>% and
# tibble() directly - dtab depends on both internally, but doesn't re-export
# either for a caller's own use.

library(dtab)
library(dplyr)   # for %>%, used throughout the examples below
library(tibble)  # for tibble(), used to build the example datasets below
...
# requiring sign-off" section; that review hasn't been redone for dtab
# specifically.
#
...
# step and returns that list instead of a huxtable. pairwise is calc_stats()'s
# own argument, passed straight through - no dispatch logic needed here,
# since pivot_crosstab()/pivot_nested_crosstab() and their format_*() partners
# already render the legend/Sig. diff row automatically whenever the data has
# sig_letter/sig_diff columns. A no-op for a Total-only table (predictors =
# NULL) - nothing to compare pairwise without a predictor.

make_table(example_data, outcomes = "approve", predictors = "region",
           statistics = "perc", conf = "se")

make_table(example_design, outcomes = "approve", predictors = "region",
           statistics = "w_perc", conf = "se", pairwise = TRUE)
...
#   dtab ported this logic without revisiting those decisions.

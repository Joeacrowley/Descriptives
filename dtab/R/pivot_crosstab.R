...
#' Reshape `calc_stats()` output into a crosstab, predictor levels as columns
#'
#' One or more FLAT predictor sets (each `predictors` element of length 1),
#' shown side by side if more than one was requested - a Sex block next to
#' an AgeGroup block, for example. For a single 2-variable nested predictor
#' set instead, use [pivot_nested_crosstab()], not this function.
#'
#' Supports `calc_stats(pairwise = TRUE)` output: a `legend` (returned as
#' the 6th list element) and `"<level>__sigdiff"` columns feed
#' [format_crosstab()]'s inserted legend row and Sig. diff row. A
#' categorical variable is limited to exactly one statistic per call;
#' a numeric variable can have more than one (e.g. mean + median).
#'
#' @param data A tibble as returned by [calc_stats()], with at least one
#'   flat predictor (`predictors` not `NULL`, and no element of length > 1).
#'
#' @return A list: `pivoted`, `bases`, `all_level_cols`, `predictor_sets`
#'   (a named vector - `cross_break` as names, display label as values),
#'   `conf_type` ("se", "ci", or `NA`), and `legend` (a tibble, or `NULL`
#'   if `pairwise = TRUE` wasn't used). Pass this list straight to
#'   [format_crosstab()].
#'
#' @seealso [format_crosstab()], [pivot_nested_crosstab()], [calc_stats()]
#' @keywords internal
...
#' Style `pivot_crosstab()` output as a huxtable
#'
#' Builds the printable crosstab: bolds variable names, inserts SE/CI rows
#' under each data row when present, inserts a Sig. diff row (and a legend
#' block above the table) when `pivot_crosstab()` was given pairwise-tested
#' data, and applies `format_statistic()`'s digit rules per row's own
#' `stat_code`.
#'
#' @param pivot_result The list returned by [pivot_crosstab()].
#'
#' @return A styled `huxtable` object, ready to print or pass to
#'   [export_table_to_excel()].
#'
#' @seealso [pivot_crosstab()]
#' @keywords internal
format_crosstab <- function(pivot_result) {
  ...
  # ---- insert one extra "sigdiff" row under every data row that was ----
  # ---- actually pairwise-tested on at least one level column - same -------
  # ---- mechanic as the conf row above, reading from "__sigdiff" instead ---
  # ---- of "__conf" ---------------------------------------------------------
  # Empty string, not NA, is how pairwise_test_one_group() marks "tested,
  # nothing came out significant" (see calc_stats.R) - that's a REAL result,
  # not a missing one, so it's treated as "has a diff row" here (it renders
  # as a blank cell on its own, no special-casing needed - see the NA-string
  # note further down). NA is different: "never tested at all" (the Total
  # column, an ineligible stat, a row-type other than data) - that's the
  # only case this still skips, so a table only ever shows a Sig. diff row
  # for rows pairwise = TRUE genuinely ran against, but always shows it for
  # those (even when nothing turned out significant) - a row silently
  # missing its Sig. diff row is otherwise indistinguishable from "wasn't
  # tested", which is misleading in a report someone else is reading without
  # the code in front of them. (Previously required at least one REAL,
  # non-empty difference before showing the row at all - caught as a real
  # design problem, not intentional, once a genuinely non-significant
  # example made the row vanish entirely.)
  if (has_pairwise) {
    data <- map_df(seq_len(nrow(data)), function(i) {
      row <- data[i, , drop = FALSE]
      diff_values <- unlist(row[, sigdiff_cols, drop = FALSE])
      has_diff <- row$row_type == "data" && any(!is.na(diff_values))
  ...
  # for).
  #
  # Total is the one column deliberately EXCLUDED from "-" on a sigdiff row
  # specifically (still gets "-" like every other level column on a data/
  # conf row, where it's a real, meaningful gap). add_pairwise_sig() never
  # compares Total against anything (cross_break != "Total" is its own
  # eligible-rows filter - see calc_stats.R), so Total's sigdiff cell is
  # ALWAYS NA, structurally, not occasionally-missing the way a real level's
  # would be - showing "-" there read as "should have a letter but doesn't",
  # when the honest answer is "this column was never part of the
  # comparison", which blank (matching a real level that WAS tested and
  # found nothing) reads as correctly. Caught as a real inconsistency, not
  # a style preference: the empty-string "nothing significant" cells on the
  # very same row already render blank, so a "-" next to them specifically
  # on Total read as a mismatched, alarming outlier rather than what it
  # actually was.
  non_total_level_idx <- level_col_idx[names(ht)[level_col_idx] != "Total"]

  ht <- ht %>%
    huxtable::set_na_string(value = "") %>%
    huxtable::set_na_string(row = c(data_rows, conf_rows), col = level_col_idx, value = "-") %>%
    huxtable::set_na_string(row = sigdiff_rows, col = non_total_level_idx, value = "-")

...
# project. Run with:
#   testthat::test_file("tests/testthat/test-make_table.R"), or devtools::test()
...
# ever consulted).
# Tests 10-12 cover pairwise = TRUE, added once make_table() actually gained
# the argument (previously calc_stats()'s own pairwise = TRUE had no path
# through make_table() at all). Same identical()-against-a-hand-chained-call
# strategy as everything else here - make_table() has no dispatch logic of
# its own for pairwise, it's just passed straight through to calc_stats(),
# so these tests exist to catch make_table() ever silently dropping the
# argument on its way through, not to re-verify add_pairwise_sig()'s own
# arithmetic (that's test_add_pairwise_sig.R's job). Test 12 checks the
# documented no-op case directly: pairwise = TRUE against a Total-only table
# (predictors = NULL) has nothing to compare across, so it should produce
# byte-identical output to pairwise = FALSE, not just "not error".
# =============================================================================
...
test_that("10. make_table(pairwise = TRUE) with a flat predictor passes pairwise through to calc_stats(), rendering the legend/Sig. diff row", {
  data <- tibble(
    approve = factor(c("Yes", "Yes", "No", "No", "Yes", "No"),
                      levels = c("No", "Yes")),
    region  = factor(c("North", "North", "North", "South", "South", "South"))
  )

  result   <- make_table(data, outcomes = "approve", predictors = "region",
                          statistics = "perc", conf = "se", pairwise = TRUE)
  expected <- format_crosstab(pivot_crosstab(
    calc_stats(data, outcomes = "approve", predictors = "region",
               statistics = "perc", conf = "se", pairwise = TRUE)
  ))
  ...
  # A real behavioural check, not just identical() against itself: without
  # pairwise, this table has no Sig. diff row/legend at all - confirming
  # pairwise = TRUE actually changed something, not just that make_table()
  # returns *a* huxtable either way.
  no_pairwise <- make_table(data, outcomes = "approve", predictors = "region",
                             statistics = "perc", conf = "se", pairwise = FALSE)
  expect_false(identical(result, no_pairwise))
})

test_that("11. make_table(pairwise = TRUE) with a nested predictor passes pairwise through to calc_stats()", {
  data <- tibble(
    age       = c(10, 20, 30, 40, 50, 60, 70, 80),
    sex       = factor(c("Male", "Male", "Male", "Male", "Female", "Female", "Female", "Female"),
                        levels = c("Male", "Female")),
  ...
  expected <- format_nested_crosstab(pivot_nested_crosstab(
    calc_stats(data, outcomes = "age", predictors = list(c("sex", "age_group")),
               statistics = "mean", conf = "se", pairwise = TRUE)
  ))
...
test_that("12. make_table(pairwise = TRUE) is a documented no-op for a Total-only table (predictors = NULL)", {
  data <- tibble(age = c(10, 20, 30, 40, 50, 60))

  with_pairwise    <- make_table(data, outcomes = "age", statistics = "mean", pairwise = TRUE)
  without_pairwise <- make_table(data, outcomes = "age", statistics = "mean", pairwise = FALSE)

  # cross_break is "Total" on every row here (no predictor to compare across),
  # so add_pairwise_sig()'s own eligible-rows filter (cross_break != "Total")
  # finds nothing to test - byte-identical output either way, not just "ran
  # without error".
  expect_identical(with_pairwise, without_pairwise)
})

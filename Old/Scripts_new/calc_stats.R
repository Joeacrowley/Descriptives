...
convert_multicodes <- function(data, base_info = NULL, keep = "Yes") {
  ...
  # never actually worked: convert_multicodes() is called from calc_stats()
  # as convert_multicodes(..., base_info = base, ...), so
  # substitute(base_info) inside this function always captures the literal
  # symbol `base` (calc_stats()'s OWN parameter name at that call site) -
  # never the name of whatever object the top-level caller actually passed
  # in as calc_stats(base = ...). The assign() therefore always wrote to a
  # global variable literally called "base", regardless of what the real
  # base_info object was named, silently clobbering (or creating) it in the
  # caller's session with no real connection back to the actual variable.
  # Caught while packaging this up as dtab - R CMD check's "assignments to
  # the global environment" check flags exactly this, and once traced
  # through it was never doing anything meaningful to begin with. Fixed
  # identically in dtab/R/calc_stats.R.

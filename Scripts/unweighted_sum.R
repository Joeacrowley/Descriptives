        ...
        suppressWarnings({
            output <- map(outcomes, function(cur_outcome) {
                ...
                one_total_table <- data %>% filter(!is.na(.data[[cur_outcome]])) %>% 
                  summarise(estimate = sum(!!sym(cur_outcome), 
                    na.rm = T), base = n(), sd = sd(!!sym(cur_outcome), 
                    na.rm = T), estimate_se = sd * sqrt(base), 
                ...
                one_total_table <- one_total_table %>% mutate(`:=`("{cur_outcome}", 
                  "sum")) %>% mutate(outcome = cur_outcome) %>% 
                ...
                table_for_one_outcome <- table_for_one_outcome %>% 
                  mutate(stat = "sum") %>% mutate(across(contains(c("cross_break", 
